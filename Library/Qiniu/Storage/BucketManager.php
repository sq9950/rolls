<?php
namespace Library\Qiniu\Storage;

use Library\Qiniu\Auth;
use Library\Qiniu\Config;
use Library\Qiniu\Http\Client;
use Library\Qiniu\Http\Error;
...
final class BucketManager
{
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        \Library\Qiniu\setWithoutEmpty($query, 'prefix', $prefix);
        \Library\Qiniu\setWithoutEmpty($query, 'marker', $marker);
        \Library\Qiniu\setWithoutEmpty($query, 'limit', $limit);
        \Library\Qiniu\setWithoutEmpty($query, 'delimiter', $delimiter);
    ...
    {
        $path = '/stat/' . \Library\Qiniu\entry($bucket, $key);
     ...
     * @param $key        待删除资源的文件名
     *
     * @return mixed      成功返回NULL，失败返回对象Library\Qiniu\Http\Error
     * @link  http://developer.qiniu.com/docs/v6/api/reference/rs/delete.html
     */
    public function delete($bucket, $key)
    {
        $path = '/delete/' . \Library\Qiniu\entry($bucket, $key);
    ...
    {
        $from = \Library\Qiniu\entry($from_bucket, $from_key);
        $to = \Library\Qiniu\entry($to_bucket, $to_key);
        $path = '/copy/' . $from . '/' . $to;
        list(, $error) = $this->rsPost($path);
        return $error;
    }
    ...
    {
        $resource = \Library\Qiniu\entry($bucket, $key);
        $encode_mime = \Library\Qiniu\base64_urlSafeEncode($mime);
    ...
    public function fetch($url, $bucket, $key = null)
    {
        ...
        $resource = \Library\Qiniu\base64_urlSafeEncode($url);
        $to = \Library\Qiniu\entry($bucket, $key);
    ...
    private static function oneKeyBatch($operation, $bucket, $keys)
    {
        $data = array();
        foreach ($keys as $key) {
            array_push($data, $operation . '/' . \Library\Qiniu\entry($bucket, $key));
        }
        return $data;
    }
    ...
    private static function twoKeyBatch($operation, $source_bucket, $key_pairs, $target_bucket)
    {
        ...
        foreach ($key_pairs as $from_key => $to_key) {
            $from = \Library\Qiniu\entry($source_bucket, $from_key);
            $to = \Library\Qiniu\entry($target_bucket, $to_key);
