...
final class ResumeUploader
{
    ...
    public function upload()
    {
        ...
        while ($uploaded < $this->size) {
            ...
            $crc = \Library\Qiniu\crc32_data($data);
    ...
    private function fileUrl()
    {
        ...
        $url .= '/mimeType/' . \Library\Qiniu\base64_urlSafeEncode($this->mime);
        if ($this->key != null) {
            $url .= '/key/' . \Library\Qiniu\base64_urlSafeEncode($this->key);
        }
        if (!empty($this->params)) {
            foreach ($this->params as $key => $value) {
                $val =  \Library\Qiniu\base64_urlSafeEncode($value);
                $url .= "/$key/$val";
            }
        }
        return $url;
