...
class UploadFile {//类定义开始
    ...
    private function save($file) {
        ...
        if($this->thumb && in_array(strtolower($file['extension']),array('gif','jpg','jpeg','bmp','png'))) {
            ...
            if(false !== $image) {
                ...
                $thumbPath    =  $this->thumbPath ? $this->thumbPath : dirname($filename).'/suoluetu/';
                // 生成图像缩略图
    ...
    private function check($file) {
		...
		//检测文件内容  add by gaolu@yundun.com 2015/03/04
		if(!$this->check_file_type($file)){
			$this->error = '文件类型与文件内容不符';
			return false;
		}
	...
	/**
	 * add by gaolu@yundun.com 2015/03/04
	 *检测文件后缀与文件内容是否相符
	 */
