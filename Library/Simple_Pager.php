...
class Simple_Pager {
	...
	//add
	public function getPageStr($param,$pre_title='Prev Text',$pre_data=array('class' => 'prev'),$min_param=array('class' => 'min'),$pageNumsParam=array('current' => array('style' => 'font-weight:bold;'), 'numbers' => array('class' => 'numbers')),$max_param=array('class' => 'max'),$next_title='Next Text',$next_param=array('class' => 'next')){
		$this->setParams($param);
		$this->previous($pre_title,$pre_data);
		$this->min($min_param);
		$this->pageNumbers($pageNumsParam,'&nbsp');
		$this->max($max_param);
		$this->next($next_title,$next_param);
		return $this->all('');
	}
}
