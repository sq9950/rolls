...
class Image {
     ...
     * @return string
      +----------------------------------------------------------
     */
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify', $output = true) {
        include_once(__LIBRARY__.'/Util/TpString.class.php');
        ...
        if($output){
            ...
            for ($i = 0; $i < $length; $i++) {
                imagestring($im, 5, $i * 20 + 5, mt_rand(1, 8), $randval{$i}, $stringColor);
            }
